...
describe('check getters when not empty', () => {
  beforeAll(async () => {
    await KeyValue.set('key', 'value');
    await KeyValue.set('last-epic-slugs', [
      'spirit-of-the-north-f58a66',
      'the-captain',
    ]);
  ...
  test('.values()', async () => {
    expect(await KeyValue.values()).toEqual([
      'value',
      ['spirit-of-the-north-f58a66', 'the-captain'],
  ...
  test('.all()', async () => {
    expect(await KeyValue.all()).toEqual([
      ...
      {
        key: 'last-epic-slugs',
        value: ['spirit-of-the-north-f58a66', 'the-captain'],
      },
  ...
  test('.search exact', async () => {
    ...
    expect(await KeyValue.search('test-exact-key', SearchFlag.Exact)).toEqual([
      { key: 'test-exact-key', value: 100 },
    ]);
    expect(await KeyValue.search('Test-Exact-Key', SearchFlag.Exact)).toEqual([
      { key: 'test-exact-key', value: 100 },
    ]);
  ...
  test('.search with escaping', async () => {
    ...
    expect(await KeyValue.search('% of')).toEqual([
      { key: '50% of 100', value: 50 },
    ]);
    expect(await KeyValue.search('ed_', SearchFlag.EndsWith)).toEqual([
      { key: '_underlined_', value: '# title' },
    ]);
    expect(await KeyValue.search('50%', SearchFlag.StartsWith)).toEqual([
      { key: '50% of 100', value: 50 },
    ]);
    expect(await KeyValue.search('% of 100', SearchFlag.EndsWith)).toEqual([
      { key: '50% of 100', value: 50 },
    ]);
    expect(await KeyValue.search('50% of 100', SearchFlag.Exact)).toEqual([
      { key: '50% of 100', value: 50 },
    ]);
  });
});
